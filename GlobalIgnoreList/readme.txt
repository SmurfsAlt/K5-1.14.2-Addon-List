...
Added in a few bug fixes and features from the retail version and updated for WoW hardcore Vanilla release!

Updated for latest WOTLK release

Block count for filters now update dynamically when the GUI is opened.  Previously the "blocked" count only updated when you open the UI.

The chat filter editor now shows the number of blocked for each filter on the main list of filters instead of making you go in and edit to see it.

Fixed a bug when clicking on an ignored player in the Ignore tab in the Blizzard UI

Removed some of the "retail" specific default chat filters and added a GDKP and Boosting filter for classic
