...
local function ApplicationStartup(self)
	...
	filterDefDesc[#filterDefDesc + 1]     = "Filter Boost Sales"
	filterDefFilter[#filterDefFilter + 1] = "(([contains=sell] or [contains=WTS] or [contains=buy] or [contains=WTB]) and [contains=boost])"
	filterDefActive[#filterDefActive + 1] = false

	filterDefDesc[#filterDefDesc + 1]     = "Filter GDKP"
	filterDefFilter[#filterDefFilter + 1] = "[contains=GDKP]"
	filterDefActive[#filterDefActive + 1] = false
...
local function EventHandler (self, event, sender, ...)
	...
	--print ("GIL DEBUG event=".. (event or "nil"))
	--print ("GIL DEBUG event=".. (event or "nil") .. " sender=" .. (sender or "nil"))
	
	--if (event == "CHANNEL_INVITE_REQUEST") then
		--print ("GIL DEBUG RECEIVED CHANNEL INVITE REQUEST")
	--end
